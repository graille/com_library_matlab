function [ber, EbN0dBs] = ber_evaluator(ilocal_transceiver, EbN0dBs, nb_errors, nb_workers, matfile)
  ...
  if matfile ~= false
    ...
    if nb_errors == -1
      error('You cannot use the infinite loop calculation without specifying a Matfile');
    end  
  end

  assert (~strcmpi(EbN0dBs, 'file'), 'ERROR: You specified the Eb/N0 range from a file, but no file has been specified');
  ...
  while loop_finished ~= true
    ...
    for i=1:length(EbN0dBs)
      if ismember(EbN0dBs(i), EbN0dBs_initial)
        ...
        if ber_total_errors(i) < required_nb_errors
          fprintf('\tStart Eb/N0 = %4.2f dB (iteration %d) [require %d errors] -- Started %s\n', EbN0dBs(i), loop_iter, required_nb_errors, datetime(now, 'ConvertFrom', 'datenum'));
